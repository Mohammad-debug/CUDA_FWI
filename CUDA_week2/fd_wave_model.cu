...
std::ofstream outReceiverFile, outFile; // output file to store receiver data
...
FDWaveModel::FDWaveModel(const char* configuration_file_relative_path) {
    ...
    holbergcoeff(fdorder, max_rel_err, hc);
...
void FDWaveModel::parse_configuration(const char* configuration_file_relative_path) {
    ...
    max_rel_err = reader.GetInteger("FDM", "max_relative_error", 1);
...
void FDWaveModel::forward_simulate() {
    ...
    medium.stagger_over_grid_modify(grid.nzt, grid.nxt); // Initializing density and lame's parameter over staggered grid
    //medium.stagger_over_grid(grid.nzt, grid.nxt); // Initializing density and lame's parameter over staggered grid



    // Check for dispersion and stability
    checkfd_ssg_elastic(grid.nx, grid.nz, grid.dx, grid.dt, source.fc[0], pml.npml,
        medium.rho, medium.lam, medium.mu, hc);


    // Overriding the material over staggered grid
    ...
    if (gpu_comp == true) {
      ...
      // Computational time
      //start_time = chrono::steady_clock::now(); // updating the start time
...
void FDWaveModel::fwi_simulate() {
    ...
    real_sim step_length;
    real_sim** We;
    real_sim C_lam, C_mu, C_rho;
...
real_sim FDWaveModel::calculate_l2_adjoint_sources(unsigned int nr, int nt, real_sim dt,
    real_sim** signal, real_sim** signal_meas) {
    // nr: number of receivers
    // nt: number of time steps
    // 
    real_sim misfit, diff;
    misfit = 0.0;

    // The receiver signal will be modified
    std::cout << "misfit = " << misfit << std::endl;
    for (unsigned int ir = 0; ir < nr; ir++) {
        for (int it = 0; it < nt; it++) {
            //std::cout << misfit << ", s1: "<< signal[ir][it] << ", s2: "<<signal_meas[ir][it]<<std::endl;
            diff = signal[ir][it] - signal_meas[ir][it];

            misfit += 0.5 * dt * diff * diff;

            signal[ir][it] = diff;

            if (fabs(diff) > 1.0e+10) {
                std::cout << " SIGNAL exceptinally high. Discrepancy = " << diff << std::endl;
                std::cout << it << ", " << ir << ", " << signal[ir][it] << ", " << signal_meas[ir][it] << std::endl;
                exit(0);
            }
...
void FDWaveModel::scale_gradients_with_energy_weights(real_sim** Ws, real_sim** Wr, real_sim C_rho, real_sim C_lam, real_sim C_mu,
    int nz, int nx, int z1, int x1, int fdz, int fdx) { // Energy weights of source and receivers combined

    real_sim** We;
    real_sim epsilon_We = 0.005;
    real_sim max_We;

    max_We = 0.0;

    allocate_array_2d(We, nz, nx);

    for (int iz = z1; iz < nz; iz = iz + fdz) {
        for (int ix = x1; ix < nx; ix = ix + fdx) {

            We[iz][ix] = sqrt(Ws[iz][ix] * Wr[iz][ix]);
    ...
    // Regularize energy weighting to avoid division by zero
    for (int iz = z1; iz < nz; iz = iz + fdz) {
        for (int ix = x1; ix < nx; ix = ix + fdx) {
    ...
    for (int iz = z1; iz < nz; iz = iz + fdz) {
        for (int ix = x1; ix < nx; ix = ix + fdx) {
            ...
            fwi.grad_lam[iz][ix] = fwi.grad_lam[iz][ix] / (We[iz][ix] * C_lam * C_lam);
            fwi.grad_mu[iz][ix] = fwi.grad_mu[iz][ix] / (We[iz][ix] * C_mu * C_mu);
            fwi.grad_rho[iz][ix] = fwi.grad_rho[iz][ix] / (We[iz][ix] * C_rho * C_rho);

        }
    }

    deallocate_array_2d(We, nz);

    std::cout << "Energy Weights:" << std::endl;
    std::cout << "epsilon: " << epsilon_We << ", max We: " << max_We << ", C_rho: " << C_rho << std::endl;
